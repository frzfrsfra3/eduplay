...
class GamedownloadsController extends Controller {
    ...
    public function store(Request $request) {
        try {
            ...
            return redirect()->route('gamedownloads.gamedownload.index')->with('success_message', 'Gamedownload was successfully added!');
        }
    ...
    public function update($id, Request $request) {
        try {
            ...
            return redirect()->route('gamedownloads.gamedownload.index')->with('success_message', 'Gamedownload was successfully updated!');
        }
        catch(Exception $exception) {
            return back()->withInput()->withErrors(['unexpected_error' => 'Unexpected error occurred while trying to process your request!']);
        }
    ...
    public function destroy($id) {
        try {
            $gamedownload = Gamedownload::findOrFail($id);
            $gamedownload->delete();
            return redirect()->route('gamedownloads.gamedownload.index')->with('success_message', 'Gamedownload was successfully deleted!');
        }
        catch(Exception $exception) {
            return back()->withInput()->withErrors(['unexpected_error' => 'Unexpected error occurred while trying to process your request!']);
        }
    ...
    protected function getData(Request $request) {
        $rules = ['user_id' => 'required', 'game_id' => 'required', 'download_type' => 'required', ];
