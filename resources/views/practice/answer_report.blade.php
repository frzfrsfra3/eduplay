    ...
    <div class="container">
        ...
        <div class="col-lg-11">
            <nav aria-label="tp-breadcm" class="tp-breadcm">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('reports') }}">@lang('practice.view_report')</a></li>
                    <li class="breadcrumb-item active" aria-current="page">@lang('practice.practice_report')</li>
                </ol>
            </nav>
            <div class="clearfix"></div>
            <div class="mdl_space text-ar-right">
                <h4 class="mrgn-bt-10">{{Auth::user()->name}}</h4>
                <span>@lang('practice.datatime') : {{ $dateTime['created_at'] }} </span>
                <div class="clearfix"></div>
            </div>
            <div class="exercise_reprt center-block">
             @foreach($practiceresult as $key => $result)
                  <input type="hidden" class="json_question" value="{{$result->question->paramRenderQuestion()}}" data-answerId="{{$result->answer_id}}">
             @endforeach
